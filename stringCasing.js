...
const sentenceCaps = (string) => {
  return string.split(' ').map(word => toCaps(word)).join(" ")
}

String.prototype.capatlizeSentence = function() {
  return this.split(' ').map(word => toCaps(word)).join(" ")
}

const everyOtherCaps = (string) => {
  return string.split("").map((letter, i) => i % 2 === 0 ? letter.toUpperCase() : letter ).join("")
}

String.prototype.evenCaps = function() {
  return this.split("").map((letter, i) => i % 2 === 0 ? letter.toUpperCase() : letter ).join("")
}

String.prototype.oddCaps = function() {
  return this.split("").map((letter, i) => i % 2 === 1 ? letter.toUpperCase() : letter ).join("")
}
...
const removeExtraWhitespace = (string) => {
  return string.split("").map((letter, i) =>  (string[i+1] === " " && letter === " ") ? "" : letter ).join("").trim()
}
...
String.prototype.camelCase = function() {
  return this.removeExtraWhitespace().capatlizeSentence().split(" ").join("").lower()
}
